...
class fgateLstmUnit(object):
    def __init__(self, hidden_size, input_size, field_size, scope_name):
        ...
        with tf.compat.v1.variable_scope(scope_name):
            ...
            self.b1 = tf.compat.v1.get_variable('b1', [2*hidden_size], initializer=tf.zeros_initializer(), dtype=tf.float32)
    ...
    def __call__(self, x, fd, s, finished = None):
        ...
        i, j, f, o = tf.split(tf.compat.v1.nn.xw_plus_b(x, self.W, self.b), 4, 1)
        r, d = tf.split(tf.compat.v1.nn.xw_plus_b(fd, self.W1, self.b1), 2, 1)
        # Final Memory cell
